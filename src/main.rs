...
async fn main() {
    ...
    let config = config::read(args.config.clone(), log_level).expect("Error while reading config file");
    let tautulli = tautulli::Tautulli::new(config.tautulli.address, config.tautulli.api_key);
    let sonarr = sonarr::Sonarr::new(config.sonarr.address, config.sonarr.api_key);
